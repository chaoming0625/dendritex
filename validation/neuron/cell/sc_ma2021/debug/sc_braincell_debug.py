...
class SC:
    ...
    def _paint_ions(self) -> None:
        ...
        needs_ca = any(
            getattr(toggles, name)
            for name in ("cav2p1", "cav3p2", "cav3p3", "kca1p1", "kca2p2")
        )
        if toggles.cdp:
    ...
    def _paint_channels(self) -> None:
        ...
        temp = u.celsius2kelvin(self.config.temperature_celsius)
        cav2p1_class = "Cav2p1_RI2021_SC_Frozen" if self.frozen else "Cav2p1_RI2021_SC"
        ...
        if t.cav2p1:
            self._paint_region_channel("soma", cav2p1_class, "Cav2p1_soma", self.params.soma.cav2p1, self._ca_name("soma"), temp, permeability=True)
            self._paint_region_channel(
                "dendprox",
                cav2p1_class,
                "Cav2p1_dendprox",
                self.params.dendprox.cav2p1,
                self._ca_name("dendprox"),
                temp,
                permeability=True,
            )
            self._paint_region_channel(
                ...
                self._ca_name("denddist"),
                temp,
                permeability=True,
            )
        if t.cav3p2:
            self._paint_region_channel("soma", "Cav3p2_RI2021_SC", "Cav3p2_soma", self.params.soma.cav3p2, self._ca_name("soma"), temp)
            self._paint_region_channel(
                "dendprox",
                "Cav3p2_RI2021_SC",
                "Cav3p2_dendprox",
    ...
    def _paint_cav3p3(self, region_name: str, instance_name: str, params: Any, temp: Any) -> None:
        ...
        self.cell.paint(
            ...
            mech.Channel(
                ...
                temp=temp,
                ion_name=self._ca_name(region_name),
            ),
        )
    ...
    def _paint_kca(
        ...
        self._paint_region_channel(
            ...
            instance_name,
            value,
            {"k": k_name, "ca": self._ca_name(region_name)},
            temp,
        )
...
def _region_cm(region: str) -> float:
    return DEND_CM_UF_CM2 if region.startswith("dend") else 1.0
