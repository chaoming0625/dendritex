...
def default_sweep_output_dir(
    *,
    ...
    return Path(cable_root) / "results" / "sweeps" / config_id
...
def default_config_run_output_dir(
    *,
    ...
    return Path(cable_root) / "results" / "config_runs" / config_name
...
def save_case_plot(out_path: str | Path, result: dict[str, Any]) -> None:
    ...
    for index in range(neuron_voltage.shape[1]):
        axes[0].plot(time_ms, neuron_voltage[:, index], linewidth=1.0, alpha=0.75)
    axes[0].set_title("NEURON all-CV voltage traces")
    axes[0].set_ylabel("Voltage (mV)")

    for index in range(braincell_voltage.shape[1]):
        axes[1].plot(time_ms, braincell_voltage[:, index], linewidth=1.0, alpha=0.75)
    axes[1].set_title("braincell all-CV voltage traces")
    axes[1].set_ylabel("Voltage (mV)")

    axes[2].plot(time_ms, np.sum(neuron_voltage, axis=1), linewidth=1.5, label="NEURON")
    axes[2].plot(time_ms, np.sum(braincell_voltage, axis=1), linewidth=1.3, linestyle="--", label="braincell")
    axes[2].set_title("All-CV voltage sum")
    axes[2].set_ylabel("Voltage sum (mV)")
    axes[2].legend()

    axes[3].plot(time_ms, abs_error.mean(axis=1), linewidth=1.5, label="mean |delta V|")
    axes[3].plot(time_ms, abs_error.max(axis=1), linewidth=1.5, label="max |delta V|")
    axes[3].set_title("Aggregate midpoint voltage error over time")
    axes[3].set_xlabel("Time (ms)")
    axes[3].set_ylabel("|delta V| (mV)")
    axes[3].legend()
...
def save_sweep_summary_plot(
    ...
    *,
    ...
    grouped = _group_metric_rows_by_observable(ok_rows, metric=metric)
    if len(grouped) > 0:
...
def save_observable_metric_boxplots(
    ...
    *,
    ...
    for axis, metric in zip(axes, metrics):
        ...
        _maybe_set_log_scale(axis, data)
...
def build_config_observable_summary_payload(
    *,
    ...
    return {
        ...
        "n_total_cases": sum(_as_optional_int(row.get("n_total_cases")) or 0 for row in template_runs),
        "n_success_cases": sum(_as_optional_int(row.get("n_success_cases")) or 0 for row in template_runs),
        "n_failed_cases": sum(_as_optional_int(row.get("n_failed_cases")) or 0 for row in template_runs),
...
def save_boxplot_by_observable_family(
    ...
    *,
    ...
    for axis, metric in zip(axes.flat, metrics):
        grouped = _group_metric_rows_by_observable(successful_rows, metric=metric)
        if len(grouped) == 0:
            axis.text(0.5, 0.5, "No successful cases.", ha="center", va="center")
            axis.set_axis_off()
            continue
...
def _aggregate_observable_group(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "rel_mae_pct_mean": (
            sum(float(row.get("rel_mae_pct_mean", 0.0)) * int(row.get("n_cases", 0)) for row in rows) / total_cases
        ),
    }
...
def _group_metric_rows_by_observable(rows: list[dict[str, Any]], *, metric: str) -> dict[str, list[float]]:
    grouped: dict[str, list[float]] = {}
    for row in rows:
        value = row.get(metric)
        if value in ("", None):
            continue
        grouped.setdefault(str(row["observable"]), []).append(float(value))
    return grouped


def _maybe_set_log_scale(axis, data: list[list[float]]) -> None:
