...
`reset()` 则丢弃运行时、数值覆盖、训练根与绑定，恢复原声明期形态引用和参数；
`reset_state()` 保留它们并在求导范围内重算动态初态。CellView 的初始电压和阈值覆盖
使用独立 State，重复编译后的数值修改仍从当前值读取。
离散预览统一经 `discretize()` 构建，live View 保存网格版本；
具体调用条件见 [生命周期](api.md#生命周期)。
